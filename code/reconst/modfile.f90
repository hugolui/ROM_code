...
module mod_field
  ...
  integer(kind=4)  :: idxi_val      ! index initial
  integer(kind=4)  :: idxf_val      ! index final
  integer(kind=4)  :: idxr_val      ! index rate
  integer(kind=4)   :: nsnap
  integer(kind=4)   :: nsnap_val
  integer(kind=4)   :: nzones, jmaxInput
  integer(kind=4)  :: model      ! model


  logical            :: logical_primitive
  logical            :: removePlunge, corrFlag, svdFlag, additionalCorrelationFlag, CGNSFlag
  ...
  type pinto
    ...
    real(kind=8), allocatable, dimension(:,:,:,:) :: r_val
    real(kind=8), allocatable, dimension(:,:,:,:) :: u_val
    real(kind=8), allocatable, dimension(:,:,:,:) :: v_val
    real(kind=8), allocatable, dimension(:,:,:,:) :: w_val
    real(kind=8), allocatable, dimension(:,:,:,:) :: p_val

    real(kind=8), allocatable, dimension(:,:,:,:) :: r_reconst
    real(kind=8), allocatable, dimension(:,:,:,:) :: u_reconst
    real(kind=8), allocatable, dimension(:,:,:,:) :: v_reconst
    real(kind=8), allocatable, dimension(:,:,:,:) :: w_reconst
    real(kind=8), allocatable, dimension(:,:,:,:) :: p_reconst

    real(kind=8), allocatable, dimension(:,:,:,:) :: diff_p
